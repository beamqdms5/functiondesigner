...
const CustomDndTables = ({ initialColumns, onColumnsChange }) => {
    ...
    const addColumn = (newColumn) => {
        ...
        setColumns([...columns, { id: newId, ...newColumn, order: columns.length + 1, visibşe: false }]);
    };
                ...
                body: {
                    wrapper: (props) => (
                        <DroppableBody
                            ...
                            data={data}
                            fromTable={fromTable}
                            moveRow={(fromIndex, toIndex, fromTable, toTable) => moveRow(fromIndex, toIndex, fromTable, toTable, visibleColumn, hiddenColumn, setColumns)}
                        />
                    ),
                    row: DraggableRow
                }
